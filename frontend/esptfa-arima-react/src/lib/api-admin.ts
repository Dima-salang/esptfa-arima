...
export const updateStudent = async (lrn: string, data: any) => {
  const response = await api.patch(`/student/${lrn}/`, data);
  return response.data;
};

export const deleteStudent = async (lrn: string) => {
  const response = await api.delete(`/student/${lrn}/`);
  return response.data;
};
