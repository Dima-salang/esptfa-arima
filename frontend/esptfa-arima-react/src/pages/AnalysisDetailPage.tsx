...
const InterpretationsCard = ({ data }: { data: AnalysisDetails }) => {
    ...
    // 1. Trend Analysis
    const faMeans = data.formative_assessments.map(fa => fa.mean / fa.max_score);
    if (faMeans.length >= 2) {
        let totalChange = 0;
        let changeCount = 0;
        for (let i = 1; i < faMeans.length; i++) {
            if (faMeans[i - 1] > 0) {
                totalChange += (faMeans[i] - faMeans[i - 1]) / faMeans[i - 1];
                changeCount++;
            }
        }
        ...
        if (Math.abs(avgChange) < 1) {
            interpretations.push({
                ...
                content: "Your class is maintaining consistent performance; scores are staying relatively stable across assessments.",
                type: 'info'
            });
        } else if (avgChange > 5) {
            interpretations.push({
                title: "Strong Growth",
                content: "Excellent! Your class is showing strong improvement; scores are clearly increasing over time.",
                type: 'success'
            });
        } else if (avgChange > 0) {
            interpretations.push({
                title: "Gradual Improvement",
                content: "Your class is making steady progress; scores are slightly increasing from earlier to later assessments.",
                type: 'success'
            });
        } else if (avgChange < -5) {
            interpretations.push({
                title: "Concerning Decline",
                content: "Attention needed: class scores are decreasing significantly. Consider reviewing recent topics.",
                type: 'error'
            });
        } else {
            interpretations.push({
                title: "Slight Decline",
                content: "There's a minor downward trend in class performance. A quick review of recent material might help.",
                type: 'warning'
            });
        }
    }
    ...
    const passRate = (data.student_performance.filter(s => (s.passing_rate || 0) >= 75).length / data.student_performance.length) * 100;
    if (passRate < 60) {
        interpretations.push({
            title: "Critical Passing Rate",
            content: `Only ${passRate.toFixed(1)}% of the class is maintaining a passing average. Broad remedial intervention is recommended.`,
            type: 'error'
        });
    } else if (passRate < 75) {
        interpretations.push({
            title: "Sub-optimal Mastery",
            content: `Currently, ${passRate.toFixed(1)}% of students are passing. Target the bottom quartile for specific support.`,
            type: 'warning'
        });
    } else if (passRate > 90) {
        interpretations.push({
            title: "High Achievement Level",
            content: `Outstanding! ${passRate.toFixed(1)}% of your students are meeting or exceeding targets.`,
            type: 'success'
        });
    }

    // 3. Toughest Topic
    const toughest = [...data.formative_assessments].sort((a, b) => a.passing_rate - b.passing_rate)[0];
    if (toughest && toughest.passing_rate < 70) {
        interpretations.push({
            title: "Struggle Point Identified",
            content: `The topic "${toughest.fa_topic_name || `FA${toughest.formative_assessment_number}`}" has the lowest mastery rate (${toughest.passing_rate.toFixed(1)}%). Consider reteaching this specific module.`,
            type: 'warning'
        });
    }
    ...
    const avgPredictedScore = validPredictions.length > 0
        ? validPredictions.reduce((acc, s) => acc + (s.predicted_score || 0), 0) / validPredictions.length
        : 0;
    const maxScore = data.document.post_test_max_score || 60;
    const predictPercent = (avgPredictedScore / maxScore) * 100;

    if (predictPercent < 75) {
        interpretations.push({
            title: "ARIMA Risk Alert",
            content: `The model predicts a post-test average of ${predictPercent.toFixed(1)}%, which is below target. Strategic intervention is needed before the final assessment.`,
            type: 'error'
        });
    } else if (predictPercent > 85) {
        interpretations.push({
            title: "Positive Model Outlook",
            content: `The ARIMA model predicts a strong post-test average of ${predictPercent.toFixed(1)}% based on current formative trends.`,
            type: 'success'
        });
    }
        ...
        <Card className="border-none shadow-md ring-1 ring-slate-200 rounded-3xl overflow-hidden bg-white">
            <CardHeader className="pb-4">
                <CardTitle className="text-xl font-black flex items-center gap-2">
                    ...
                    Automated Class Interpretations
                    <Badge className="bg-indigo-100 text-indigo-700 border-none px-2 py-0.5 text-[10px]">AI-DRIVEN</Badge>
                </CardTitle>
                <CardDescription>Synthesized insights from your class performance data and ARIMA predictions.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
                ...
                {interpretations.length === 0 && (
                    <div className="py-10 text-center space-y-2">
                        ...
                        <p className="text-slate-400 text-sm font-medium">Add more assessment data to generate automated interpretations.</p>
                    </div>
                )}
            </CardContent>
        </Card>
