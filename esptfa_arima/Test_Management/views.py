...
from arima_model.visualization_manager import (
    generate_heatmap,
    generate_student_line_chart,
    generate_score_dist_chart,
    generate_boxplot,
    generate_bar_chart,
    generate_student_vs_class_chart,
    generate_student_comparison_chart,
)
...
class FormativeAssessmentDetailView(
    LoginRequiredMixin, TeacherRequiredMixin, DetailView
):
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            if not document.status:
                messages.info(
                    request,
                    message="The document is still being processed. Please check back later by refreshing the page.",
                )
            ...
            messages.error(
                request,
                "An error occurred while accessing the document. Please try again.",
            )
            return redirect("formative_assessment_dashboard")
    ...
    def get_context_data(self, **kwargs):
        try:
            ...
            try:
                assessments = list(
                    FormativeAssessmentScore.objects.filter(analysis_document=document)
                )
                test_topics = list(
                    TestTopicMapping.objects.filter(
                        analysis_document=document
                    ).order_by("test_number")
                )
                individual_fas = list(
                    FormativeAssessmentStatistic.objects.filter(
                        analysis_document=document
                    )
                )
                student_stats = StudentScoresStatistic.objects.filter(
                ...
                messages.error(
                    self.request, "Error loading assessment data. Please try again."
                )
                return redirect("formative_assessment_dashboard")

            if not assessments:
                messages.warning(
                    self.request, "No assessment data found for this document."
                )
                return context

            last_fa = len(individual_fas)
            context["last_fa"] = last_fa
            context["last_fa_scores"] = FormativeAssessmentScore.objects.filter(
                analysis_document=document,
                formative_assessment_number=context["last_fa"],
            )

            try:
                document_statistic = AnalysisDocumentStatistic.objects.filter(
                    analysis_document=document
                ).first()
                        ...
                        all_scores_data.append(
                            {
                                ...
                                "normalized_scores": assessment.score
                                / (assessment.passing_threshold / 0.75),
                            }
                        )

                    if all_scores_data:
                        processed_data = pd.DataFrame(all_scores_data)
                        processed_data["test_number"] = processed_data[
                            "test_number"
                        ].astype(int)
                        ...
                        raw_overall_insights, overall_insights = (
                        ...
                        raw_heatmap_insight_data, heatmap_insights = (
                            get_visualization_insights(
                                "heatmap",
                                processed_data,
                        ...
                        try:
                            # get gemini insights
                            gemini_insights_obj = (
                                AnalysisDocumentInsights.objects.filter(
                                    analysis_document=document
                                ).first()
                            )
                        ...
                        if "actionable" in overall_insights:
                            context["actionable_insights"].extend(
                                overall_insights["actionable"]
                            )
                        ...
                        if "outliers" in heatmap_insights:
                            context["high_performers"] = heatmap_insights[
                                "outliers"
                            ].get("high_performers", [])
                            context["low_performers"] = heatmap_insights[
                                "outliers"
                            ].get("low_performers", [])
                            context["inconsistent_performers"] = heatmap_insights[
                                "outliers"
                            ].get("inconsistent_performers", [])
                except Exception as e:
                    logger.error(f"Error generating insights: {e}")
                    messages.warning(
                        self.request,
                        "Could not generate insights. Some features may be limited.",
                    )
                    context["insights"] = {}
                    context["gemini_insights"] = None
                    context["actionable_insights"] = []
            ...
            try:
                score_distribution_data = self.prepare_score_distribution_data(
                    assessments
                )
                ...
                messages.warning(
                    self.request, "Could not generate score distribution chart."
                )
                context["score_distribution_data"] = {
                    "labels": ["No Data"],
                    "data": [0],
                }
            ...
            try:
                test_performance_data = self.prepare_test_performance_data(
                    individual_fas, assessments
                )
                context["test_performance_data"] = test_performance_data
            except Exception as e:
                logger.error(f"Error preparing test performance data: {e}")
                messages.warning(
                    self.request, "Could not generate test performance chart."
                )
            ...
            try:
                ...
                context["predictions"] = PredictedScore.objects.filter(
                    ...
                    gap_to_passing=ExpressionWrapper(
                        F("passing_threshold") - F("score"), output_field=FloatField()
                    ),
                )
                ...
                messages.warning(
                    self.request,
                    "Some data could not be loaded. Some features may be limited.",
                )
                context["assessments"] = []
                context["predictions"] = []
                context["test_topics"] = []
            ...
            try:
                ...
                for score in context["last_fa_scores"]:
                    ...
                    statistic = student_stats.filter(
        ...
        try:
            ...
            data = [
                sum(
                    1
                    for assessment in assessments
                    if ranges[i][0] <= assessment.score < ranges[i][1]
                )
                for i in range(5)
            ]
...
class IndividualFADetailView(LoginRequiredMixin, TeacherRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["normalized_mean_scaled"] = (
            fa_statistic.mean / fa_statistic.max_score * 100
            if fa_statistic.max_score
            else 0
        )
        ...
        fa_data = pd.DataFrame(
            list(
                context["formative_scores"].values(
                    "student_id__student_id",
        ...
        if not fa_data.empty:
            ...
            if "actionable" in distribution_insights:
                context["actionable_insights"].extend(
                    distribution_insights["actionable"]
                )
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            with transaction.atomic():
                try:
                    ...
                    fa_data = preprocessed_data[
                        preprocessed_data["test_number"] == fa_number
                    ]

                    # generate charts at runtime
                    histogram_image = generate_score_dist_chart(fa_data, fa_number)
                    histogram_filename = (
                        f"histogram_{analysis_document.pk}_{fa_number}.png"
                    )
                    ...
                    bar_chart_filename = (
                        f"bar_chart_{analysis_document.pk}_{fa_number}.png"
                    )

                    # Generate student comparison chart
                    student_comparison_chart = generate_student_comparison_chart(
                        fa_data, None, fa_number
                    )
                    student_comparison_filename = (
                        f"student_comparison_{analysis_document.pk}_{fa_number}.png"
                    )

                    # save images
                    fa_statistic.histogram.save(
                        histogram_filename,
                        ContentFile(histogram_image.read()),
                        save=True,
                    )
                    ...
                    fa_statistic.bar_chart.save(
                        bar_chart_filename,
                        ContentFile(bar_chart_image.read()),
                        save=True,
                    )
                    fa_statistic.student_comparison_chart.save(
                        student_comparison_filename,
                        ContentFile(student_comparison_chart.read()),
                        save=True,
                    )
                except Exception as e:
            ...
            messages.error(
                request, "Unable to access the requested formative assessment."
            )
            return redirect("formative_assessment_dashboard")
...
class IndividualStudentDetailView(LoginRequiredMixin, TeacherRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not student_data.empty:
            ...
            student_data["normalized_scores"] = (
                student_data["score"] / student_data["max_score"]
            )
            ...
            try:
                    ...
                    for topic, group in topics_df.groupby("topic"):
                        ...
                        if avg_score >= 90:
                            topic_insights.append(
                                f"Strong performance in {topic} (avg {avg_score:.1f}%)"
                            )
                        elif avg_score >= 75:
                            topic_insights.append(
                                f"Good understanding of {topic} (avg {avg_score:.1f}%)"
                            )
                        elif avg_score >= 60:
                            topic_insights.append(
                                f"Average performance in {topic} (avg {avg_score:.1f}%)"
                            )
                        else:
                            topic_insights.append(
                                f"Needs improvement in {topic} (avg {avg_score:.1f}%)"
                            )
                ...
                context["insights"] = {"line_chart": line_insights}
                ...
                if all_scores.exists():
                    ...
                    class_data = pd.DataFrame(
                        list(all_scores.values("test_number", "score"))
                    )
                ...
                if (
                    "comparison" in context["insights"]
                    and "actionable" in context["insights"]["comparison"]
                ):
                ...
                logger.info(
                    f"Generated insights for student {student_statistic.student.student_id}"
                )
            except Exception as e:
    ...
    def get_object(self, queryset=None):
        try:
            ...
            if (
                not student_statistic.heatmap
                or not student_statistic.lineplot
                or not student_statistic.performance_comparison_chart
            ):
                with transaction.atomic():
                    ...
                    try:
                        ...
                        student_data = preprocessed_data[
                            preprocessed_data["student_id"] == student_id
                        ]
                        ...
                        heatmap_filename = (
                            f"student_heatmap_{analysis_document.pk}_{student_id}.png"
                        )

                        lineplot = generate_student_line_chart(
                            student_data, analysis_document
                        )
                        lineplot_filename = (
                            f"student_line_plot_{analysis_document.pk}_{student_id}.png"
                        )

                        # Generate comparison chart
                        performance_comparison_chart = None
                        # Get all scores for the document (for class comparison)
                        class_data = preprocessed_data[
                            preprocessed_data["student_id"] != student_id
                        ]
                        if not class_data.empty:
                            class_data.rename(
                                columns={"formative_assessment_number": "test_number"},
                                inplace=True,
                            )
                        ...
                        student_statistic.heatmap.save(
                            heatmap_filename,
                            ContentFile(heatmap_image.read()),
                            save=True,
                        )
                        ...
                        if performance_comparison_chart:
                            student_statistic.performance_comparison_chart.save(
                                comparison_filename,
                                ContentFile(performance_comparison_chart.read()),
                                save=True,
                            )
...
def delete_document(request, document_pk):
    ...
    try:
        ...
        document.delete()
        messages.success(
            request, f"Document '{document_title}' was successfully deleted."
        )
    except Exception as e:
        logger.error(f"Error deleting document: {e}")
        messages.error(
            request, f"An error occurred while deleting the document: {str(e)}"
        )
...
def delete_document_ajax(request, document_pk):
    ...
    try:
        ...
        return JsonResponse(
            {
                "status": "success",
                "message": f"Document '{document_title}' was successfully deleted.",
            }
        )
    except Exception as e:
